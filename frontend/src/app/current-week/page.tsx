...
import { fetchCurrentSchedule, fetchTeams } from '@/lib/api';
...
export default function CurrentWeekPage() {
    ...
    useEffect(() => {
        ...
        async function init() {
            try {
                ...
                setTeams(teamsData);
                // Default to first team or previously selected
    ...
    useEffect(() => {
        async function load() {
            setLoading(true);
            try {
                const scheduleData = await fetchCurrentSchedule(selectedTeamId || undefined);
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold mb-2 flex items-center gap-3">
                        <Calendar className="text-primary w-8 h-8" /> Current Week Analysis
                    </h1>
            ...
            <div className="bg-primary/10 border border-primary/20 rounded-2xl p-4 flex gap-3 text-sm text-primary">
                <Info className="w-5 h-5 flex-shrink-0" />
                <p>The <b>Schedule Tracker</b> calculates total games for your roster and your opponent within the matchup window. Use this to identify streaming opportunities.</p>
            </div>
